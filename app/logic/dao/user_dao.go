...
func (dao *UserDao) GetByIdAndCache(id int64) (bool, *models.UcUser) {
	...
	exists, err := dao.connect.orm.Id(id).Get(UcUser)
	if exists {
...
func (dao *UserDao) GetById(id int64) (boolean bool, userModel *models.UcUser) {
	user := &models.UcUser{}
	exists, err := dao.connect.orm.Id(id).Get(user)
	helper.CheckErr(err)
	return exists, user
}
...
func (dao *UserDao) GetByEmail(email string) (boolean bool, userModel *models.UcUser) {
	user := &models.UcUser{}
	exists, err := dao.connect.orm.Where("email=? and status=?", email, 1).Get(user)
	helper.CheckErr(err)
	return exists, user
}
...
func (dao *UserDao) IsExistsByEmail(email string) bool {
	user := &models.UcUser{}
	countNum, err := dao.connect.orm.Where("email=? and status=?", email, 1).Count(user)
	helper.CheckErr(err)
	if countNum > 0 {
		return true
	}
...
func (dao *UserDao) CreateUser(user *models.UcUser) (boolean bool, id int64) {
	id, err := dao.connect.orm.InsertOne(user)
